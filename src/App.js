...
import { Brain, TrendingUp, AlertCircle, Calendar, Sparkles, ArrowRight, BarChart3, Heart, Coffee, Moon, Target, Zap, Clock, Download, Share2, Sun, Volume2, VolumeX, BarChart2, Eye } from 'lucide-react';
...
const cancelBooking = (bookingId) => {
  ...
  if (view === 'admin') {
    ...
    const icons = {
      total: <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="#9333ea" strokeWidth="2" fill="#f3e8ff" /><text x="12" y="16" textAnchor="middle" fontSize="12" fill="#9333ea">Σ</text></svg>,
      avg: <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><rect x="4" y="10" width="16" height="8" rx="4" fill="#f59e42" /><text x="12" y="17" textAnchor="middle" fontSize="12" fill="#fff">AVG</text></svg>,
      min: <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><rect x="4" y="14" width="16" height="4" rx="2" fill="#22c55e" /><text x="12" y="18" textAnchor="middle" fontSize="12" fill="#fff">MIN</text></svg>,
      max: <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><rect x="4" y="6" width="16" height="12" rx="4" fill="#ef4444" /><text x="12" y="15" textAnchor="middle" fontSize="12" fill="#fff">MAX</text></svg>,
      trend: <svg width="28" height="28" fill="none" viewBox="0 0 24 24"><path d="M4 16l6-6 4 4 6-8" stroke="#6366f1" strokeWidth="2" fill="none" /><circle cx="20" cy="8" r="2" fill="#6366f1" /></svg>
    };
    return (
      <div className="admin-container" style={{maxWidth: 1100, margin: '40px auto', padding: 24}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'1.5rem',marginBottom:'2.5rem'}}>
          <div style={{background:'#f3e8ff',borderRadius:14,padding:'1.5rem',display:'flex',alignItems:'center',gap:16,boxShadow:'0 2px 8px #9333ea22'}}>
            {icons.total}
            <div>
              <div style={{fontSize:'1.3rem',fontWeight:700,color:'#9333ea'}}>{adminStats?.total ?? '-'}</div>
              <div style={{color:'#9333ea',fontWeight:500}}>Total Records</div>
            </div>
          </div>
          <div style={{background:'#fff7ed',borderRadius:14,padding:'1.5rem',display:'flex',alignItems:'center',gap:16,boxShadow:'0 2px 8px #f59e4222'}}>
            {icons.avg}
            <div>
              <div style={{fontSize:'1.3rem',fontWeight:700,color:'#f59e42'}}>{adminStats?.avgIntensity ?? '-'}</div>
              <div style={{color:'#f59e42',fontWeight:500}}>Avg Intensity</div>
            </div>
          </div>
          <div style={{background:'#f0fdf4',borderRadius:14,padding:'1.5rem',display:'flex',alignItems:'center',gap:16,boxShadow:'0 2px 8px #22c55e22'}}>
            {icons.min}
            <div>
              <div style={{fontSize:'1.3rem',fontWeight:700,color:'#22c55e'}}>{adminStats?.minIntensity ?? '-'}</div>
              <div style={{color:'#22c55e',fontWeight:500}}>Min Intensity</div>
            </div>
          </div>
          <div style={{background:'#fef2f2',borderRadius:14,padding:'1.5rem',display:'flex',alignItems:'center',gap:16,boxShadow:'0 2px 8px #ef444422'}}>
            {icons.max}
            <div>
              <div style={{fontSize:'1.3rem',fontWeight:700,color:'#ef4444'}}>{adminStats?.maxIntensity ?? '-'}</div>
              <div style={{color:'#ef4444',fontWeight:500}}>Max Intensity</div>
            </div>
          </div>
          <div style={{background:'#eef2ff',borderRadius:14,padding:'1.5rem',display:'flex',alignItems:'center',gap:16,boxShadow:'0 2px 8px #6366f122'}}>
            {icons.trend}
            <div>
              <div style={{fontSize:'1.3rem',fontWeight:700,color:'#6366f1'}}>{adminStats?.trend ?? '-'}</div>
              <div style={{color:'#6366f1',fontWeight:500}}>Overall Trend</div>
            </div>
          </div>
        </div>

        {/* Trend Chart Placeholder */}
        <div style={{background:'#fff',borderRadius:16,boxShadow:'0 2px 16px #0001',padding:'2rem 1.5rem',marginBottom:'2.5rem',minHeight:120}}>
          <h3 style={{fontSize:'1.2rem',marginBottom:16,borderLeft:'4px solid #9333ea',paddingLeft:8,color:'#9333ea'}}>Trend Chart</h3>
          <div style={{height:80,display:'flex',alignItems:'center',justifyContent:'center',color:'#aaa',fontStyle:'italic',fontSize:'1.1rem'}}>
        ...
        <div style={{display:'flex',gap:'2rem',flexWrap:'wrap',marginBottom:'2.5rem'}}>
          {/* Records Card */}
          <div style={{flex:'2 1 500px',background:'#fff',borderRadius:16,boxShadow:'0 2px 16px #0001',padding:'2rem 1.5rem',minWidth:320}}>
            <h3 style={{fontSize:'1.2rem',marginBottom:16,borderLeft:'4px solid #9333ea',paddingLeft:8,color:'#9333ea'}}>Recent Records</h3>
            {/* User List */}
            <div style={{marginBottom:16,display:'flex',flexWrap:'wrap',alignItems:'center',gap:'0.5rem'}}>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:'1rem',background:'#fafaff',borderRadius:12,overflow:'hidden',boxShadow:'0 1px 4px #0001'}}>
                ...
                <tbody>
                  {adminRecords && adminRecords.length > 0 ? adminRecords.slice(-20).reverse().map((rec, i) => (
                    <tr key={i} style={{background: i%2===0 ? '#fff' : '#f6f6fa', transition:'background 0.2s', cursor:'pointer'}} onClick={()=>{setSelectedUser({user:rec.user || rec.email || rec.username || 'Unknown',records:[rec]});setShowUserModal(true);}}>
